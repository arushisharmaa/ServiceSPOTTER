...
struct ShelterDetailView: View {
    var organization: ShelterOrganizationItems
    
    @State var showFavoritesOnly = false
    @ObservedObject var favorites2 = Favorites2()
        ...
        VStack(alignment: .center, spacing:25 , content: {
            ScrollView(.vertical, showsIndicators: /*@START_MENU_TOKEN@*/true/*@END_MENU_TOKEN@*/, content: {
                ...
                HStack {
                    ...
                    Button(favorites2.contains(organization) ? "Unlike" : "Like")
                            {
                        if self.favorites2.contains(self.organization) {
                            self.favorites2.remove(self.organization)
                        } else {
                            self.favorites2.add(self.organization)
                        }
...
struct ShelterDetailView_Previews: PreviewProvider {
    static var previews: some View {
        ShelterDetailView(organization: ShelterOrganizationItemsList.shelterFirstTen.first!)
    }
}
